...
class SpectralConv2d(nn.Module):
  ...
  def __call__(self, x):
    ...
    center_spect = x_ft[:, -self.mode1+center_idx:self.mode1+center_idx, :self.mode2,:]

    # Multiply the center of the spectrum by the kernel
    x_filt = jnp.einsum('bijc,coij->bijo', x_ft, kernel)

    # Pad the kernel with zeros to restore the original size
    pad_size_1 = (x_filt.shape[1] - self.mode1)//2
    pad_size_2 = (x_filt.shape[2] - self.mode2)//2
    x_filt = jnp.pad(
      x_filt, 
...
class FNO2D(nn.Module):
  ...
  def get_grid(x):
    ...
    grid = jnp.expand_dims(jnp.stack([x1, x2], axis=-1), -1)
